...
export const getMe = async (): Promise<LoginUserInfo> => {
  const response = await new GetApiRequestBuilder<LoginUserInfo>('me', 'me.get').sendRequest()
...
export const deleteUser = async (): Promise<void> => {
  await new DeleteApiRequestBuilder('me', 'me.delete').sendRequest()
}
...
export const updateDisplayName = async (displayName: string): Promise<void> => {
  await new PostApiRequestBuilder<void, ChangeDisplayNameDto>('me/profile', 'me.setDisplayName')
    .withJsonBody({
      displayName
    })
    .sendRequest()
...
export const getMyMedia = async (): Promise<MediaUpload[]> => {
  const response = await new GetApiRequestBuilder<MediaUpload[]>('me/media', 'me.listMedias').sendRequest()
  return response.asParsedJsonObject()
}
